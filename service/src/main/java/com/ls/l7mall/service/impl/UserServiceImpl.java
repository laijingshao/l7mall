...
import com.ls.l7mall.dao.UserDao;
import com.ls.l7mall.entity.User;
import com.ls.l7mall.global.Const;
import com.ls.l7mall.global.ResponseEntity;
import com.ls.l7mall.global.TokenCache;
import com.ls.l7mall.service.UserService;
import com.ls.l7mall.util.MD5Util;
...
public class UserServiceImpl implements UserService {
    ...
    public ResponseEntity<String> resetPassword(String oldPassword, String newPassword, User user) {
        // 判断原密码是否正确
        int resultCount = userDao.checkPasword(user.getId(), MD5Util.MD5EncodeUtf8(oldPassword));
        if(resultCount == 0){
            return ResponseEntity.responesWhenError("原密码错误");
        }
        // 重置密码
        resultCount = userDao.updatePassword(user.getUsername(), MD5Util.MD5EncodeUtf8(newPassword));
        if(resultCount > 0 ){
            return ResponseEntity.responesWhenSuccess("重置密码成功");
        }
        return ResponseEntity.responesWhenError("重置密码失败");
    ...
    public ResponseEntity<User> updateInformation(User user) {
        ...
        int resultCount = userDao.checkEmailById(user.getId(),user.getemail());
        if(resultCount > 0){
            return ResponseEntity.responesWhenError("email已被使用");
        }
        // 更新信息
        ...
        int updateCount = userDao.updateById(updateUser);
        if(updateCount > 0){
